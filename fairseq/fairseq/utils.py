...
def my_replace_unk(hypo_str, src_str, alignment, align_dict, unk):
    from fairseq import tokenizer
    # Tokens are strings here
    hypo_tokens = tokenizer.tokenize_line(hypo_str)
    # TODO: Very rare cases where the replacement is '<eos>' should be handled gracefully
...
def post_process_prediction(hypo_tokens, src_str, alignment, align_dict, tgt_dict, remove_bpe):
    ...
    if align_dict is not None and len(align_dict) > 0:
    ...
    if align_dict is not None and src_str and alignment.size(0) > 0: #use attention positions and source to replace unks
       hypo_str = my_replace_unk(hypo_str, src_str, alignment, align_dict, tgt_dict.unk_string())

    return hypo_tokens, hypo_str, alignment


def make_positions(tensor, padding_idx, left_pad, prefix=0, decside=False):
