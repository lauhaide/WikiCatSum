# Laura Perez
# Based on fconv.py hierarchical decoder with topic label (key) prediction
...
class HKVPredFairseqModel2(HKVPredFairseqModel):
    ...
    def forward(self, tgt_key, src_keywords, src_tokens, src_lengths, prev_output_tokens):
        ...
        dec_input['tgt_key'] = self.get_target_keys__(tgt_key, prev_output_tokens).view(b, -1)
...
@register_model('fconv_fatte')
class FEncFairseqModel(HKVPredFairseqModel2):
    def __init__(self, docEncoder, docDecoder, lambda_keyloss):
        ...
        self.lambda_keyloss = lambda_keyloss
    ...
    def add_args(parser):
        ...
        parser.add_argument('--lambda-keyloss', type=float, default=1, metavar='D',
                            help='hyperparameter to template coverage loss.')
...
class LDocumentDecoder(nn.Module):
    ...
    def __init__(
        ...
        # topic label (key) out for prediction, as a sequence of topic keys for prediction, add padding and eos to the nb of keys
        self.state2key = Linear(hidden_size, num_topics + 3)
    ...
    def forward(self, decoder_in_dict, encoder_out_dict,
        ...
        return dec_outs, (attn_scores, word_atte_scores), new_incr_cached, tkeys
    ...
    def forward(self, prev_output_tokens, h0, sent_pos_emb, encoder_out_dict=None, incremental_state=None, firstfeed=False, normpos=False):
        ...
        if self.embed_positions is not None:
            ...
            if incremental_state is not None:
                ...
                pos_embed += sent_pos_emb.unsqueeze(1).repeat(1, pos_embed.size(1), 1)
            if normpos:
...
@register_model_architecture('fconv_fatte', 'fconv_fatte')
...
@register_model_architecture('fconv_fatte', 'fconv_fatte_wikicatsum_big')
...
@register_model_architecture('fconv_fatte', 'fconv_fatte_iwslt_de_en')
def fconv_iwslt_de_en(args):
    ...
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 256)
    args.firstfeed = getattr(args, 'firstfeed', False)
    base_architecture(args)


@register_model_architecture('fconv_fatte', 'fconv_fatte_wikicatsum')
def fconv_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
    args.encoder_layers = getattr(args, 'encoder_layers', '[(256, 3)] * 4')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 256)
