...
def collate(samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False, dummy=False, use_prefix=False):
    ...
    if samples[0].get('target', None) is not None:
        target = merge_flat('target', left_pad=left_pad_target)
        # we create a shifted version of targets for feeding the
        # previous output token(s) into the next decoder step
        prev_output_tokens = merge_flat(
            'target',
    ...
    # there is no target keys in flat, we neither use source topic distribution
...
class WikiCatSumPairDatasetTopicsFlatdec(WikiCatSumPairDataset):
    """
    ...
    def __init__(
        ...
        max_source_positions=15, max_target_positions=1200,
        max_src_sentence_length=302, max_tgt_sentence_length=None, shuffle=True,
    ...
    def get_dummy_batch(self, num_tokens, max_positions, src_len=128, tgt_len=128, sent_len=128):
        (max_source_positions, max_ssentence_length), \
            max_target_positions = self._get_max_positions(max_positions)
        ...
        tsent_len = min(tgt_len, max_target_positions)
    ...
    def num_tokens(self, index):
        ...
        return self.src_sizes[index][0]*self.src_sizes[index][1]+ (self.tgt_sizes[index] if self.tgt_sizes is not None else 0)
    ...
    def valid_size(self, index, max_positions):
        ...
        (max_source_positions, msrclen), max_target_positions = self._get_max_positions(max_positions)
    ...
    def _get_max_positions(self, max_positions):
        ...
        (max_src_pos, max_ssent_len), max_tgt_pos = max_positions
