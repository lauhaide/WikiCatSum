...
class SingleHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if use_scalar_bias:
            ...
            v = scalar_bias(v, 1)
            tgt_size += 1  ###strage is BUG or I'm using this wrongly????? see below also usage of use_scalar_bias and keymask
            if use_scalar_bias and key_padding_mask is not None:
                key_padding_mask = torch.cat((key_padding_mask,
                                              torch.cuda.ByteTensor(key_padding_mask.size(0), 1).fill_(0)), 1)
        ...
        if key_padding_mask is not None:
            ...
            if key_padding_mask.max() > 0:
                if self.downsample:
                    ...
                    if use_scalar_bias and key_padding_mask is not None:
                        attn_weights = attn_weights.view(size, self.num_heads, tgt_len, src_len+1)
                    else:
                ...
                if use_scalar_bias and key_padding_mask is not None:
                    attn_weights = attn_weights.view(size, tgt_len, src_len+1)
                else:
